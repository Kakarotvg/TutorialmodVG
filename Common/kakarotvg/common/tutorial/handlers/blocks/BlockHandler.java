package kakarotvg.common.tutorial.handlers.blocks;
...
public class BlockHandler {
    ...
    public static void configureBlocks(Configuration config) {
        tutorialblock = new MetaBlock(BlockIDs.tutorialblockID, Material.iron).setUnlocalizedName("tutorialblock").setCreativeTab(CreativeTabHandler.ttab).setStepSound(Block.soundMetalFootstep).setHardness(50.0F).setResistance(2000.0F);
        animatedblock = new TutorialBlock(BlockIDs.animatedblockID, Material.wood).setUnlocalizedName("animatedblock").setCreativeTab(CreativeTabHandler.ttab).setStepSound(Block.soundClothFootstep).setHardness(1.0F).setResistance(4.0F);
    }
